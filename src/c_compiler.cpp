    ...
    try {
        if (argc >= 3) {
            ...
            if (!(infile = fopen(SRC_FILE.c_str(), "r"))) {
                throw std::runtime_error("[ERROR] Cannot open source file '" + SRC_FILE);
            }
            ...
            if (MODE_FLAG == "-S") {
                ...
                throw std::runtime_error("[ERROR] Unidentified flag " + MODE_FLAG + ", unable to set driver");
            }
        ...
        throw std::runtime_error("[ERROR] Received " + std::to_string(argc) + " arguments when minimum required is 3");
