...
f:
		...
		nop
		sw	$t8, 100($fp)		# (fn args) Store fn call args from $t8 to memory

if_start_1:
		addiu	$sp, $sp, -8 		# (eval expr) move sp for virtual regs
		li	$t0, 0				# (int const)
		sw	$t0, -8($fp) 		# (eval expr) store RHS in virtual
		lw	$t0, 100($fp)		# (var: normal) Reading from variable "n"
		nop
...
if_next_1:
		...
		nop
		addiu	$sp, $sp, 0		# Erasing virtual register for f
		sw	$v0, 0($fp) 		# (eval expr) store RHS in virtual
		lw	$t0, 100($fp)		# (var: normal) Reading from variable "n"
		nop
		lw	$t1, 0($fp) 		# (eval expr) load LHS from virtual to $t1, RHS in $t0
		nop
		addiu	$sp, $sp, 8 		# (eval expr) clearing virtual
...
fmain:
		...
		.set noreorder
		.set reorder
		addiu	$sp, $sp, -68		# (fn def: frame start) Move sp to end of new frame
		sw	$fp, 0($sp) 		# (fn def) Store addr of old fp on stack
		add	$fp, $sp, $0 		# (fn def) Move fp to new sp
		sw	$ra, 4($sp) 		# (fn def) Store ra on stack
		sw	$s0, 8($sp) 		# (fn def) Store save regs $s0-$s7 on stack
...
fmain_end_2:
		...
		lw	$fp, 0($fp) 		# (fn def) Load prev fp into $fp
		nop
		addiu	$sp, $sp, 68		# (fn def: frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
