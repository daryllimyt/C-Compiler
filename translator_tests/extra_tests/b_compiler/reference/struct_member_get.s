...
f:
		...
		li	$t0, 13				# (int const)
		sw	$t0, 0($fp)			# (struct: attribute) Writing to struct "z" , attribute "y" of struct type "x"
		nop
		lw	$t0, 0($fp)			# (struct: attribute) Reading from struct "z" , attribute "y" of struct type "x"
		nop
