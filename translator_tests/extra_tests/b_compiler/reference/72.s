...
fmain:
		...
		nop
		li	$t0, 1				# (int const)
		sw	$t0, 140($fp)		# (assign) store var result in NORMAL variable "a"
		li	$t0, 2				# (int const)
		sw	$t0, 132($fp)		# (assign) store var result in NORMAL variable "b"
		li	$t0, 3				# (int const)
		sw	$t0, 124($fp)		# (assign) store var result in NORMAL variable "c"
		li	$t0, 4				# (int const)
		sw	$t0, 116($fp)		# (assign) store var result in NORMAL variable "d"
		li	$t0, 5				# (int const)
		sw	$t0, 108($fp)		# (assign) store var result in NORMAL variable "e"
		lw	$t0, 140($fp)		# (var: normal) Reading from variable "a"
		nop
