...
sub:
		...
		addiu	$sp, $sp, -24		# Move $sp to end of function arg section for local variables
		...
		addiu	$sp, $sp, -64	# Move $sp to end of variable section before function call
		addiu	$sp, $sp, -8 		# (eval expr) Expand stack for expression evaluation
		lw	$t0, -8($fp)			# (var: normal) Reading from variable "j"
		nop
		...
		addiu	$sp, $sp, -8 		# (eval expr) Expand stack for expression evaluation
		lw	$t0, -16($fp)			# (var: normal) Reading from variable "k"
		nop
		sw	$t0, 0($sp) 		# (eval expr) store RHS in memory
		lw	$t0, 0($fp)			# (var: normal) Reading from variable "i"
		nop
...
sub_end_0:
		addiu	$sp, $sp, 88	# Move $sp to end of function arg section after function call
...
fmain:
		...
		sw	$ra, 8($sp) 		# (fn def)
		move	$fp, $sp 		# $fp is at the start of the variable section
		addiu	$sp, $sp, -40	# Move $sp to end of variable section before function call
		lw	$t0, 8($fp)	# (var: array) Reading from array "arr" at index 1
		nop
		addiu	$sp, $sp, -32	# (fn call) Expand stack for fn "sub" arg slots
		li	$t0, 4				# (int const)
		...
		nop
		addiu	$sp, $sp, 32	# (fn call) Shrink stack for fn "sub" arg slots
		sw	$v0, 8($fp)			# (assign) store fn result in ARRAY variable "arr"
		li	$t0, 0				# (int const)
		move	$v0, $t0 		# (return node) load $t0 to $v0 if not function call
		j	fmain_end_1
		nop
...
fmain_end_1:
		addiu	$sp, $sp, 40	# Move $sp to end of function arg section after function call
		move	$sp, $fp 		# Restore sp to start of variable section
		lw	$fp, 4($sp) 		# (fn def)
		lw	$ra, 8($sp) 		# (fn def)
		addiu	$sp, $sp, 12		# (fn def: frame end) Shrink stack back to previous frame
