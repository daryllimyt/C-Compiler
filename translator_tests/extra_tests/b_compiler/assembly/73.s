...
f1:
		...
		nop
		sw	$t8, 76($fp)		# (fn args) Store fn call args from $t8 to new virtual
...
fmain:
		...
		.cpload $t4
		.set reorder
		addiu	$sp, $sp, -212		# (fn def: frame start) Move sp to end of new frame
		sw	$fp, 0($sp) 		# (fn def) Store addr of old fp on stack
		add	$fp, $sp, $0 		# (fn def) Move fp to new sp
		sw	$ra, 4($sp) 		# (fn def) Store ra on stack
		sw	$s0, 8($sp) 		# (fn def) Store save regs $s0-$s7 on stack
		...
		lw	$t0, 204($fp)		# (var: normal) Reading from variable "a"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -8($fp) 		# (fn call params) store in virtual
		lw	$t0, 196($fp)		# (var: normal) Reading from variable "b"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		...
		lw	$t0, 188($fp)		# (var: normal) Reading from variable "c"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		...
		lw	$t0, 180($fp)		# (var: normal) Reading from variable "d"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -48($fp) 		# (fn call params) store in virtual
		lw	$t0, 172($fp)		# (var: normal) Reading from variable "e"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
...
fmain_end_2:
		...
		lw	$fp, 0($fp) 		# (fn def) Load prev fp into $fp
		nop
		addiu	$sp, $sp, 212		# (fn def: frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
