...
f:
		...
		nop
		sw	$t8, 172($fp)		# (fn args) Store fn call args from $t8 to new virtual
		lw	$t8, -16($t9) 		# (fn args) Load fn call args from old virtual to $t8
		nop
		sw	$t8, 164($fp)		# (fn args) Store fn call args from $t8 to new virtual
		...
		lw	$t8, -56($t9) 		# (fn args) Load fn call args from old virtual to $t8
		nop
...
fmain:
		...
		.cpload $t4
		.set reorder
		addiu	$sp, $sp, -244		# (fn def: frame start) Move sp to end of new frame
		sw	$fp, 0($sp) 		# (fn def) Store addr of old fp on stack
		add	$fp, $sp, $0 		# (fn def) Move fp to new sp
		sw	$ra, 4($sp) 		# (fn def) Store ra on stack
		sw	$s0, 8($sp) 		# (fn def) Store save regs $s0-$s7 on stack
		...
		nop
		li	$t0, 1				# (int const)
		sw	$t0, 236($fp)		# (assign) store var result in NORMAL variable
		li	$t0, 2				# (int const)
		sw	$t0, 228($fp)		# (assign) store var result in NORMAL variable
		li	$t0, 3				# (int const)
		sw	$t0, 220($fp)		# (assign) store var result in NORMAL variable
		li	$t0, 4				# (int const)
		sw	$t0, 212($fp)		# (assign) store var result in NORMAL variable
		li	$t0, 5				# (int const)
		sw	$t0, 204($fp)		# (assign) store var result in NORMAL variable
		li	$t0, 6				# (int const)
		sw	$t0, 196($fp)		# (assign) store var result in NORMAL variable
		li	$t0, 7				# (int const)
		sw	$t0, 188($fp)		# (assign) store var result in NORMAL variable
		lw	$t0, 236($fp)		# (var: normal) Reading from variable "a"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -8($fp) 		# (fn call params) store in virtual
		lw	$t0, 228($fp)		# (var: normal) Reading from variable "b"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -16($fp) 		# (fn call params) store in virtual
		lw	$t0, 220($fp)		# (var: normal) Reading from variable "c"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -24($fp) 		# (fn call params) store in virtual
		lw	$t0, 212($fp)		# (var: normal) Reading from variable "d"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -32($fp) 		# (fn call params) store in virtual
		lw	$t0, 204($fp)		# (var: normal) Reading from variable "e"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -40($fp) 		# (fn call params) store in virtual
		lw	$t0, 196($fp)		# (var: normal) Reading from variable "x"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -48($fp) 		# (fn call params) store in virtual
		lw	$t0, 188($fp)		# (var: normal) Reading from variable "y"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
...
fmain_end_1:
		...
		lw	$fp, 0($fp) 		# (fn def) Load prev fp into $fp
		nop
		addiu	$sp, $sp, 244		# (fn def: frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
