...
f:
.globl f
		addiu	$sp, $sp, -132 		# (frame start) Move sp to end of new frame
...
f_end_0:
		...
		nop
		addiu	$sp, $sp, 132 		# (frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
...
fmain:
.globl fmain
		addiu	$sp, $sp, -164 		# (frame start) Move sp to end of new frame
		sw	$fp, 0($sp) 		# Store addr of old fp on stack
		add	$fp, $sp, $0 		# Move fp to new sp
		sw	$ra, 4($sp) 		# Store ra on stack
		sw	$s0, 8($sp) 		# Store save regs $s0-$s7 on stack
...
fmain_end_1:
		...
		lw	$fp, 0($fp) 		# Load prev fp into $fp
		nop
		addiu	$sp, $sp, 164 		# (frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
