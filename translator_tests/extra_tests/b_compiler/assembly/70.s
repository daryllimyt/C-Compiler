...
f:
		...
		lw	$t0, 100($fp)		# (var: normal) Reading from variable "d"
		nop
		sw	$t0, -8($fp) 		# (eval expr) store lhs in virtual
		addiu	$sp, $sp, -8 		# (eval expr) move sp for virtual regs
		lw	$t0, 108($fp)		# (var: normal) Reading from variable "c"
		nop
		sw	$t0, -16($fp) 		# (eval expr) store lhs in virtual
		addiu	$sp, $sp, -8 		# (eval expr) move sp for virtual regs
		lw	$t0, 116($fp)		# (var: normal) Reading from variable "b"
		nop
		sw	$t0, -24($fp) 		# (eval expr) store lhs in virtual
		lw	$t0, 124($fp)		# (var: normal) Reading from variable "a"
		nop
