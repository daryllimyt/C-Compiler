...
for_start_1:
		...
		lw	$t0, 132($fp)		# (var: normal) Reading from variable "i"
		nop
		...
		sw	$t0, 140($fp)		# (assign expr) storing evaluated expression from $t0 to LHS variable in memory

for_continue_1:
		lw	$t0, 132($fp)		# (var: normal) Reading from variable "i"
		nop
		lw	$t0, 132($fp)		# (postfix) store var result in NORMAL variable
		nop
		move	$t1, $t0
		addi	$t0, $t0, -1
		sw	$t0, 132($fp)
