...
fmain:
		...
		.cpload $t4
		.set reorder
		addiu	$sp, $sp, -156		# (fn def: frame start) Move sp to end of new frame
		...
		lw	$t0, 140($fp)		# (var: normal) Reading from variable "b"
		nop
		sw	$t0, -8($fp) 		# (eval expr) store lhs in virtual
		lw	$t0, 148($fp)		# (var: normal) Reading from variable "a"
		nop
		...
		sw	$t0, 148($fp)		# (assign expr) storing evaluated expression from $t0 to LHS variable in memory
		addiu	$sp, $sp, -8 		# (eval expr) move sp for virtual regs
		lw	$t0, 132($fp)		# (var: normal) Reading from variable "c"
		nop
		sw	$t0, -8($fp) 		# (eval expr) store lhs in virtual
		lw	$t0, 148($fp)		# (var: normal) Reading from variable "a"
		nop
