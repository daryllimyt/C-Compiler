...
f:
		...
		nop
		sw	$t8, 100($fp)		# (fn args) Store fn call args from $t8 to new virtual
		...
		lw	$t0, 92($fp)		# (var: normal) Reading from variable "z"
		nop
		sw	$t0, -8($fp) 		# (eval expr) store lhs in virtual
		addiu	$sp, $sp, -8 		# (eval expr) move sp for virtual regs
		lw	$t0, 100($fp)		# (var: normal) Reading from variable "y"
		nop
...
fmain:
		...
		lw	$t0, 140($fp)		# (var: normal) Reading from variable "x"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -8($fp) 		# (fn call params) store in virtual
		lw	$t0, 132($fp)		# (var: normal) Reading from variable "y"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
		sw	$t0, -16($fp) 		# (fn call params) store in virtual
		lw	$t0, 124($fp)		# (var: normal) Reading from variable "z"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
