...
f:
.globl f
		addiu	$sp, $sp, -100 		# (frame start) Move sp to end of new frame
		...
		nop
		lw	$t0, 92($fp) 		# Reading from variable
		nop
		li	$t0, 10
		li	$t0, 10
...
f_end_0:
		...
		nop
		addiu	$sp, $sp, 100 		# (frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
...
.globl fmain
		...
		nop
		li	$t0, 1
		sw	$t0, 92($fp)		# (assign) store var result
		...
		nop
		sw	$v0, 108($fp)		# (assign) store function result
		lw	$t0, 92($fp) 		# Reading from variable
		nop
		add	$v0, $t0, $0 		# (return node) put return val in $v0
		j	fmain_end_1
