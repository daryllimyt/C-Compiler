...
f:
		...
		nop
		sw	$t8, 84($fp)		# (fn args) Store fn call args from $t8 to new virtual
		addiu	$sp, $sp, -8 		# (eval expr) move sp for virtual regs
		lw	$t0, 84($fp)		# (var: normal) Reading from variable "y"
		nop
...
fmain:
		...
		lw	$t0, 108($fp)		# (var: normal) Reading from variable "y"
		nop
		addiu	$sp, $sp, -8 		# Expanding stack
