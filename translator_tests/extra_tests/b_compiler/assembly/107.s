...
fmain:
.globl fmain
		addiu	$sp, $sp, -204 		# (frame start) Move sp to end of new frame
		...
		nop
		li	$t0, 0
		sw	$t0, 196($fp)		# (assign) store var result
		li	$t0, 1
		sw	$t0, 188($fp)		# (assign) store var result
		li	$t0, 0
		sw	$t0, 180($fp)		# (assign) store var result
for_start_1
		lw	$t0, 188($fp)
		nop
		sw	$t0, 172($fp)
		beq	t0, 0, for_end_1
		nop
		lw	$t0, 196($fp) 		# Reading from variable
		nop
		li	$t0, 1
if_start_2
		...
		beq	t0, 0, if_next_2
		nop
		lw	$t0, 188($fp) 		# Reading from variable
		nop
		li	$t0, 0
		sw	$t0, 188($fp)		# (assign) store var result
		j	if_end_2
...
fmain_end_0:
		...
		nop
		addiu	$sp, $sp, 204 		# (frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
