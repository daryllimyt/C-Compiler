...
fmain:
.globl fmain
		addiu	$sp, $sp, -92 		# (frame start) Move sp to end of new frame
		...
		nop
		li	$t0, 3
		sw	$t0, 84($fp)		# (assign) store var result
		lw	$t0, 84($fp) 		# Reading from variable
		nop
...
fmain_end_0:
		...
		nop
		addiu	$sp, $sp, 92 		# (frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
