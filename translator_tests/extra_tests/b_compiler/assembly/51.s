...
fmain:
		...
		.cpload $t4
		.set reorder
		addiu	$sp, $sp, -92		# (fn def: frame start) Move sp to end of new frame
		...
		.cprestore 60
		nop
		li	$t0, 65535				# (int const)
		sw	$t0, 84($fp)		# (assign) store var result in NORMAL variable
		lw	$t0, 84($fp)		# (var: normal) Reading from variable "x"
		nop
		li	$t1, -1
xor	$t0, $t0, $t1
		sw	$t0, 84($fp)		# (unary) storing to variable
...
fmain_end_0:
		...
		nop
		addiu	$sp, $sp, 92		# (fn def: frame end) Move sp to end of previous frame
		beq	$ra, $0, end
		nop
		jr	$ra
		nop
